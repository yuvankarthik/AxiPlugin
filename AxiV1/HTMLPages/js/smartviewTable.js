...
wireDom() {
  ...
  buildParams(pageNo = 1) {
    ...
    if (this.aggregations && typeof this.aggregations === 'object' && Object.keys(this.aggregations).length) {
      props.aggregations = Object.assign({}, this.aggregations);
    }
...
attachScrollListener() {
  ...
  toggleGroupByField(fieldName) {
    ...
    this.groupby_columns = Array.isArray(this.groupby_columns) ? this.groupby_columns : [];
    const idx = this.groupby_columns.findIndex(f => String(f).toLowerCase() === field.toLowerCase());
    if (idx >= 0) this.groupby_columns.splice(idx, 1);
    else this.groupby_columns.push(field);

    if (this.groupby_columns.length > 0) {
      this.select_columns = Array.from(new Set(this.groupby_columns.map(f => String(f).trim()).filter(Boolean)));
      if (!this.aggregations || typeof this.aggregations !== 'object') this.aggregations = {};
      if (Object.keys(this.aggregations).length === 0) {
        const key = `total_${this.groupby_columns[0]}`.replace(/[^a-zA-Z0-9_]/g, '_');
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function applyGroupByFromQuery(ctrl, meta) {
    ...
    ctrl.groupby_columns = Array.from(new Set(resolved));
    ctrl.select_columns = Array.from(new Set(resolved));
    if (!ctrl.aggregations || typeof ctrl.aggregations !== 'object') ctrl.aggregations = {};
    if (Object.keys(ctrl.aggregations).length === 0) {
      const key = `total_${ctrl.groupby_columns[0]}`.replace(/[^a-zA-Z0-9_]/g, '_');
      ctrl.aggregations[key] = 'count(*)';
    }
  }
